...
def _apply_dyad(verb: Verb, left_arr: np.ndarray, right_arr: np.ndarray) -> np.ndarray:
    ...
    # For operand with longer frame, each (-rcf)-cell contains multiple operand cells.
    # For the operand with shorter frame, each (-rcf)-cell contains a single operand cell.
    cells = []
    for left_cell, right_cell in zip(
        left_arr_reshaped, right_arr_reshaped, strict=True
    ):
